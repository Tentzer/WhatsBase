...
- `embeddings` — tenant_id, ref_type (product | business_info), ref_id, content, vector vector(3072), HNSW index, plus a GIN index on a metadata jsonb column for hybrid filtering
...
`ANTHROPIC_API_KEY, OPENAI_API_KEY, SUPABASE_URL, SUPABASE_SERVICE_KEY, SUPABASE_ANON_KEY, DATABASE_URL, REDIS_URL, LANGFUSE_PUBLIC_KEY, LANGFUSE_SECRET_KEY, LANGFUSE_HOST, INTAKE_MODE, APP_BASE_URL`
